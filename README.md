<h1 align="center">Insight-api-square</h1>

<div align="center">
  <strong>A Square blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/squareevo/insight-api-square">
    <img src="https://img.shields.io/travis/squareevo/insight-api-square/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-square">
    <img src="https://img.shields.io/npm/v/insight-api-square.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/squarepay/insight-ui-square.
...
- [Resources](#resources)
- [License](https://github.com/squareevo/insight-api-square/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-square@latest
bitcore-node-square create mynode
cd mynode
bitcore-node-square install insight-api-square
bitcore-node-square start
```

The API endpoints will be available by default at: `http://localhost:3001/insight-api-square/`

### Prerequisites

- [Bitcore Node Square 3.x](https://github.com/squareevo/bitcore-node-square)

**Note:** You can use an existing Square data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `square.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-api-square/block/[:hash]
  /insight-api-square/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-square/block-index/[:height]
  /insight-api-square/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-square/rawblock/[:blockHash]
```
...
```
  /insight-api-square/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-square/tx/[:txid]
  /insight-api-square/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-square/rawtx/[:rawid]
  /insight-api-square/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-square/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-square/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-square/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-square/addr/[:addr]/balance
  /insight-api-square/addr/[:addr]/totalReceived
  /insight-api-square/addr/[:addr]/totalSent
  /insight-api-square/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-square/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-square/addrs/[:addrs]/utxo
  /insight-api-square/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-square/addrs/utxo
```
...
```
  /insight-api-square/txs/?block=HASH
  /insight-api-square/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-square/txs/?address=ADDR
  /insight-api-square/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-square/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-square/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-square/addrs/txs
```
...
GET method:
```
  /insight-api-square/gobject/list/proposal
```
...
GET method:
```
  /insight-api-square/gobject/get/[:hash]
  /insight-api-square/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-square/masternodes/list
```
### Validate Masternode
```
  /insight-api-square/masternodes/validate/[:payee]
  /insight-api-square/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-square/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-square/peer
```

### Status of the Bitcoin Network
```
  /insight-api-square/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-square/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a Square Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
